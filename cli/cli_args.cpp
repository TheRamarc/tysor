...
std::variant<std::pair<std::string, std::vector<std::int64_t>>, Diagnostic> parse_shape_spec(
    ...
    if (dims.empty()) {
        return cli_error("shape '" + spec + "' must contain at least one dimension");
    }
...
std::string usage() {
    return
        "cpptysor <input.ty> [options]\n"
        "\n"
        "Options:\n"
        "  --run                         Run the entry function with the selected backend\n"
        "  --backward                    Run the backward pass for supported programs\n"
